...
"""
Node to detect Characters with a stationary light source. 
Gets signal to start character recognition through publishing of PlateROI from main and ocr/start True
...
class CharDetector:
    def __init__(self) -> None:
        ...
        self.plate_detector = PlateDetector(
            "object-detection/model/best.pt"
        )
    ...
    def perform_image_preprocessing(self, image):
        ...
        # If image is too dark add some edge enhancement on top
        if img_brightness < 100:
            sharpened_image = cv2.equalizeHist(sharpened_image)
        # If image too bright invert the image
    ...
    def ocr_start_callback(self, msg):
        rospy.loginfo("Start or Stop OCR")
