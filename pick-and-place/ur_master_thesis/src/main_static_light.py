...
class AutomationPipeline:
    def __init__(self):
        ...
        self.center_point_pub2 = rospy.Publisher(
            "/plate/center2", PlateROI, queue_size=10
        )
    ...
    def start(self):
        ...
        if self.PICK_SUCCESS == 0:
            ...
            if self.HAS_CHARS == False and self.TURN_ME == False:
                ...
                self.position_controller.go_to_pose_intermediate(
                    aruco_translation, aruco_quat
                )
    ...
    def pick_up_plate_for_light(self):
        self.gripper.move_and_wait_for_pos(0, 10, 10)
        if self.HAS_CHARS == False and self.TURN_ME == False:
            point_mean, y_axis, x_axis = (
                self.position_controller.deliver_go_to_light_position_rot(self.gripper)
            )
        else:
            point_mean, y_axis, x_axis = (
                self.position_controller.deliver_go_to_light_position(self.gripper)
            )
        self.stop_ocr()
    ...
    def deliver_plate_for_light(self):
        ...
        ) = utils.get_ocr_roi(point_mean, y_axis, x_axis, self.WIDTH, self.LENGTH)
        self.start_ocr()
    ...
    def run(self):
        ...
        if bboxes != None:
            ...
            try:
                ...
                o3d.io.write_point_cloud(
                    self.DEBUG_FOLDER + "pcl_" + str(self.DEBUG_RUN_NUM) + ".ply", pcl
                ),
                o3d.io.write_point_cloud(
                    self.DEBUG_FOLDER + "surface_" + str(self.DEBUG_RUN_NUM) + ".ply",
                    pcl_surface,
                ),
                o3d.io.write_point_cloud(
                    self.DEBUG_FOLDER
                ...
                o3d.io.write_point_cloud(
                    self.DEBUG_FOLDER + "smooth_" + str(self.DEBUG_RUN_NUM) + ".ply",
                    pcl_surface_smooth,
                )
                o3d.io.write_triangle_mesh(
                    self.DEBUG_FOLDER
                    + "mesh_coordinate_frame_"
                    + str(self.DEBUG_RUN_NUM)
                    + ".ply",
                    mesh_coordinate_frame,
                )
                o3d.io.write_line_set(
                    self.DEBUG_FOLDER + "line_set_" + str(self.DEBUG_RUN_NUM) + ".ply",
                    line_set,
                )
                ...
                if self.MODE == 0:
                    quaternion, point_base, R_tcp, point_base_rot = self.grasp_above(
                        normal_z, normal_y, ee_pose, point_base
                    )
                else:
                    quaternion, point_base, R_tcp, point_base_rot = self.grasp_side(
                        normal_z, normal_y, ee_pose, point_base
                    )
                ...
                (
                    point_center_edge_21,
                    point_center_edge_22,
                    point_center_edge_21_inner,
                    point_center_edge_22_inner,
                ) = utils.get_ocr_roi_2(
                    point_mean, R_tcp[:, 1], R_tcp[:, 0], self.WIDTH, self.LENGTH
                )
                ...
                if self.HAS_CHARS2 == True:
                    ...
                    quaternion = np.array([-qy, qx, -qw, qz])
        ...
        try:
            ...
            for transform in msg.transforms:
                ...
                if (
                    ...
                    rospy.loginfo(
                        f"Board transformation updated: {self.aruco_position}"
                    )
        except Exception as e:
